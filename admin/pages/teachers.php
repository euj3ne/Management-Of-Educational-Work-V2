                        ...
                        <table class="table">
                            ...
                            <?php
                                foreach ($result as $row) {
                                    ...
                                    echo "<td>";
                                    echo "<a class='btn btn-primary shadow open-trash-modal-btn' role='button' href='#' data-bs-toggle='modal' data-bs-target='#modal-delete' style='background: rgb(66,70,133); border-color: #ffffff00; margin-right: 5px;' onclick='setTeachersIdToDelete(" . escape($row['id']) . ")'>";
                                    ...
                                    echo "<a class='btn btn-primary shadow open-modal-btn' role='button' href='#' data-bs-toggle='modal' data-bs-target='#modal-edit' style='background: rgb(66,70,133); border-color: #ffffff00; margin-right: 5px;' onclick='onEditModalOpen({$row['id']}, \"{$row['last_name']}\", \"{$row['first_name']}\", \"{$row['middle_name']}\", \"{$row['birth_date']}\", \"{$row['gender']}\", \"{$row['address']}\", \"{$row['phone']}\", \"{$row['email']}\", \"{$row['employment_date']}\", \"{$row['position']}\")'>";
...
<div class="modal fade" role="dialog" tabindex="-1" id="modal-delete">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="../actions/teachers/delete.php" enctype="multipart/form-data">
                <div class="modal-header">
                    <h4 class="modal-title" style="color: rgb(0,0,0);">Удалить запись?</h4>
                    <button class="btn-close" type="button" aria-label="Close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="teachers_id" id="teachers_id_to_delete">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-danger" type="submit" name="delete_teachers">Удалить</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function setTeachersIdToDelete(teachersId) {
        document.getElementById('teachers_id_to_delete').value = teachersId;
    }
</script>
...
<div class="modal fade" role="dialog" tabindex="-1" id="modal-add">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="../actions/teachers/add.php" enctype="multipart/form-data">
                <div class="modal-header">
                    <h4 class="modal-title" style="color: rgb(0,0,0);">Добавление преподавателя</h4>
                    <button class="btn-close" type="button" aria-label="Close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="add_employment_date" class="form-label">Дата трудоустройства</label>
                        <input type="date" class="form-control" id="add_employment_date" name="employment_date" required>
                    </div>
                    <div class="mb-3">
                        <label for="add_position" class="form-label">Должность</label>
                        <input type="text" class="form-control" id="add_position" name="position" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-light" type="button" data-bs-dismiss="modal">Закрыть</button>
                    <button class="btn btn-primary" type="submit" name="add_teachers">Добавить</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" role="dialog" tabindex="-1" id="modal-edit">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="../actions/teachers/edit.php" enctype="multipart/form-data">
                <div class="modal-header">
                    <h4 class="modal-title" style="color: rgb(0,0,0);">Изменение преподавателя</h4>
                    <button class="btn-close" type="button" aria-label="Close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="edit_teachers_id" name="teachers_id">
                    ...
                    <div class="mb-3">
                        <label for="edit_employment_date" class="form-label">Дата трдоустройства</label>
                        <input type="date" class="form-control" id="edit_employment_date" name="employment_date" required>
                    </div>
                    <div class="mb-3">
                        <label for="edit_position" class="form-label">Должность</label>
                ...
                <div class="modal-footer">
                    <button class="btn btn-light" type="button" data-bs-dismiss="modal">Закрыть</button>
                    <button class="btn btn-primary" type="submit" name="edit_teachers">Сохранить изменения</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function onEditModalOpen(teachersId, lastName, firstName, middleName, birthDate, gender, address, phone, email, employmentDate, position) {
